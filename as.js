...
var cloudant_password = 'zzz';

// requires that indexing has been enabled - looks up term in the database 'db' of the owner with said uname user name and upwd password.
function gen_cloudant_lookup_url(owner, db, term, uname, upwd){
...
$(function() {
	$('#as_in').keyup(function(e){
		...
		if(code == 13) { // user pressed 'ENTER'
			...
			$.getJSON('http://localhost:6969/r/' + gen_cloudant_lookup_url('dequilibrium', 'as', $('#as_in').val(), cloudant_username, cloudant_password), null,
			 function(data){
